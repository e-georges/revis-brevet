...
const MUTATEURS_PAR_CHAPITRE = {
  ...
  pc_01:    ['loi_ohm']
  // PAS de "default" : un chapitre absent de cette liste (français, histoire-géo, SVT, etc.)
  // ne doit JAMAIS recevoir de questions mutées de maths. C'était la cause du bug
  // "question de calcul d'aire affichée dans un quiz SVT".
};

function genererMutations(chapId, nb = 3) {
  const disponibles = MUTATEURS_PAR_CHAPITRE[chapId];
  if (!disponibles || !disponibles.length) return []; // Aucune mutation hors maths/sciences déclarées
  const questions = [];
  for (let i = 0; i < nb; i++) {
    const nom = disponibles[Math.floor(Math.random() * disponibles.length)];
    try {
      const fn = Mutations[nom];
      if (!fn) continue;
...
// ── OPENTDB — CULTURE GÉNÉRALE GRATUITE ──────────────────────
// 4700+ questions gratuites, sans clé, en anglais
const OPENTDB_CATEGORIES = {
  histoire:  { id: 23, label: 'Histoire 🌍' },
  geo:       { id: 22, label: 'Géographie 🗺️' },
  sciences:  { id: 17, label: 'Sciences 🔬' },
  maths:     { id: 19, label: 'Mathématiques 📐' },
  info:      { id: 18, label: 'Informatique 💻' },
  general:   { id: 9,  label: 'Culture générale 🎓' }
};
...
async function getOpentdbToken() {
  ...
  try {
    ...
    _opentdbToken = d.token;
    return _opentdbToken;
  } catch { return null; }
}

async function fetchOpentdb(catId, difficulty = 'medium', amount = 5) {
  const token = await getOpentdbToken();
  const url = `https://opentdb.com/api.php?amount=${amount}&category=${catId}&difficulty=${difficulty}&type=multiple${token ? '&token=' + token : ''}`;
  const r = await fetch(url);
  const d = await r.json();
  if (d.response_code === 4) { _opentdbToken = null; return fetchOpentdb(catId, difficulty, amount); }
  if (d.response_code !== 0) throw new Error('Pas assez de questions disponibles.');
  return d.results.map((q, i) => {
    const decode = s => { const t = document.createElement('textarea'); t.innerHTML = s; return t.value; };
    const bonne = decode(q.correct_answer);
    const opts = shuffleArr([bonne, ...q.incorrect_answers.map(decode)]);
    return {
      id: `otdb_${catId}_${Date.now()}_${i}`,
      enonce: decode(q.question),
      options: opts.map((o, j) => `${['A','B','C','D'][j]}) ${o}`),
      bonne_reponse: opts.indexOf(bonne),
      explication: `✅ Réponse : ${bonne}`,
      niveau: difficulty === 'easy' ? 1 : difficulty === 'medium' ? 2 : 3,
      _source: 'opentdb',
      langue: '🇬🇧'
    };
  });
...
function buildNav() {
  ...
  menu.innerHTML = `
    ...
    <li><a class="nav-item" id="btn-culture"><span>🌍</span><span>Culture générale</span></a></li>
...
// ── CULTURE GÉNÉRALE (OpenTDB) ────────────────────────────────
function renderCulture() {
  const c = document.getElementById('app-view-container');
  c.innerHTML = `
    <h2 style="color:var(--text-primary);margin-bottom:6px;">🌍 Culture générale</h2>
    <p style="color:var(--text-secondary);font-size:.85rem;margin-bottom:20px;">
      Questions gratuites via <strong>Open Trivia Database</strong> — aucune clé requise.<br>
      En anglais 🇬🇧 — bonus entraînement LV1 !
    </p>

    <div class="card" style="margin-bottom:20px;">
      <div style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:14px;">
        ${Object.entries(OPENTDB_CATEGORIES).map(([key, cat]) =>
          `<button class="btn-cat" data-cat="${key}" data-id="${cat.id}"
            style="background:var(--bg-app);border:1.5px solid var(--border-color);padding:7px 14px;border-radius:8px;cursor:pointer;font-size:.82rem;color:var(--text-primary);">
            ${cat.label}
          </button>`
        ).join('')}
      </div>
      <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
        <label style="font-size:.82rem;color:var(--text-secondary);">Difficulté :</label>
        <select id="otdb-diff" style="padding:6px 10px;border:1.5px solid var(--border-color);border-radius:7px;background:var(--bg-card);color:var(--text-primary);font-size:.82rem;">
          <option value="easy">Facile</option>
          <option value="medium" selected>Moyen</option>
          <option value="hard">Difficile</option>
        </select>
        <select id="otdb-nb" style="padding:6px 10px;border:1.5px solid var(--border-color);border-radius:7px;background:var(--bg-card);color:var(--text-primary);font-size:.82rem;">
    ...
    <div id="culture-zone"></div>

    <div style="background:#FEF9C3;border:1px solid #FCD34D;border-radius:10px;padding:12px 16px;margin-top:16px;font-size:.8rem;color:#713F12;line-height:1.6;">
      💡 Ces questions sont en anglais et ne sont pas calées sur le programme DNB. Elles sont idéales pour s'entraîner entre deux sessions de révisions sérieuses.
    </div>
  `;

  // Sélection catégorie active
  let catActive = { key: 'general', id: 9 };
  document.querySelectorAll('.btn-cat').forEach(btn => {
    btn.addEventListener('click', () => {
      document.querySelectorAll('.btn-cat').forEach(b => b.style.background = 'var(--bg-app)');
      btn.style.background = 'var(--bg-card-hover)';
      btn.style.borderColor = 'var(--color-primary)';
      btn.style.color = 'var(--color-primary)';
      catActive = { key: btn.dataset.cat, id: parseInt(btn.dataset.id) };
    });
  });

  // Bouton lancer
  const zone = document.getElementById('culture-zone');
  const btnLancer = document.createElement('button');
  btnLancer.className = 'btn-primary';
  btnLancer.style.cssText = 'width:100%;padding:12px;font-size:.95rem;margin-bottom:16px;';
  btnLancer.textContent = '🚀 Lancer le quiz culture générale';
  zone.appendChild(btnLancer);

  btnLancer.addEventListener('click', async () => {
    const diff = document.getElementById('otdb-diff').value;
    const nb   = parseInt(document.getElementById('otdb-nb').value);
    btnLancer.textContent = '⏳ Chargement...';
    btnLancer.disabled = true;
    try {
      const questions = await fetchOpentdb(catActive.id, diff, nb);
      AppState.quiz = {
        chapitreId: `culture_${catActive.key}`,
        matLabel: OPENTDB_CATEGORIES[catActive.key]?.label || 'Culture',
        questions, index: 0, score: 0, infini: false
      };
      afficherQuestion();
      document.getElementById('quiz-modal').classList.add('active');
    } catch(err) {
